...
> **Önce katalogda ara (zorunlu):** Üretmeden önce `skills/skills-catalog.csv` ve
> `agents/agents-catalog.csv`'yi tara (`grep`/`awk`) — gerçekten yok mu? Varsa onu kullan,
> tekrar (duplicate) üretme. Çalışma döngüsü: `catalog.md`.
...
- `model`: mekanik/yapılandırılmış iş → `haiku`, muhakeme/mimari/uzmanlık → `sonnet`,
  en karmaşık orkestrasyon → `opus` (en az token + ortalama-üstü performans). Karar kuralı: `model-selection.md`.
...
Yeni skill/agent ekledikten sonra **önce indeks/katalogu yeniden üret**, sonra yayımla:

```bash
python3 scripts/build-catalog.py     --source <.claude>   # skills/agents-catalog.csv
