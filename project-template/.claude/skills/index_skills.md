# Skill İndeksi

> `project-template/.claude/skills/` altındaki tüm skill'lerin **kategori bazlı** ad listesi.
> Plugin → alt-kategori şeklinde gruplandı (hacimli kovalarda skill adının 2. sözcüğüne göre).
> Plugin eşlemesi `scripts/build-marketplace.sh` ile birebir aynıdır. Açıklama için ilgili `SKILL.md`'ye bakın.
> Bu dosya `scripts/build-skill-index.py` ile üretilir — elle düzenlemeyin, yeniden çalıştırın.
> **Toplam:** 1612 skill · **Son tarama:** 2026-06-12

> **Plugin'ler:** [mutfak-security (754)](#mutfak-security) · [mutfak-consulting (347)](#mutfak-consulting) · [mutfak-dev (150)](#mutfak-dev) · [mutfak-marketing (103)](#mutfak-marketing) · [mutfak-pm (78)](#mutfak-pm) · [mutfak-design (99)](#mutfak-design) · [mutfak-dotnet (38)](#mutfak-dotnet) · [mutfak-diagrams (15)](#mutfak-diagrams) · [mutfak-frontend (8)](#mutfak-frontend) · [mutfak-research (4)](#mutfak-research) · [mutfak-core (16)](#mutfak-core)

---

## mutfak-security

> Savunma/blue-team güvenlik (forensics, detection, hunting, hardening, pentest) · **754 skill**

### Performing (172)

- `sec-performing-access-recertification-with-saviynt`
- `sec-performing-access-review-and-certification`
- `sec-performing-active-directory-bloodhound-analysis`
- `sec-performing-active-directory-compromise-investigation`
- `sec-performing-active-directory-forest-trust-attack`
- `sec-performing-active-directory-penetration-test`
- `sec-performing-active-directory-vulnerability-assessment`
- `sec-performing-adversary-in-the-middle-phishing-detection`
- `sec-performing-agentless-vulnerability-scanning`
- `sec-performing-ai-driven-osint-correlation`
- `sec-performing-alert-triage-with-elastic-siem`
- `sec-performing-android-app-static-analysis-with-mobsf`
- `sec-performing-api-fuzzing-with-restler`
- `sec-performing-api-inventory-and-discovery`
- `sec-performing-api-rate-limiting-bypass`
- `sec-performing-api-security-testing-with-postman`
- `sec-performing-arp-spoofing-attack-simulation`
- `sec-performing-asset-criticality-scoring-for-vulns`
- `sec-performing-authenticated-scan-with-openvas`
- `sec-performing-authenticated-vulnerability-scan`
- `sec-performing-automated-malware-analysis-with-cape`
- `sec-performing-aws-account-enumeration-with-scout-suite`
- `sec-performing-aws-privilege-escalation-assessment`
- `sec-performing-bandwidth-throttling-attack-simulation`
- `sec-performing-binary-exploitation-analysis`
- `sec-performing-blind-ssrf-exploitation`
- `sec-performing-bluetooth-security-assessment`
- `sec-performing-brand-monitoring-for-impersonation`
- `sec-performing-clickjacking-attack-test`
- `sec-performing-cloud-asset-inventory-with-cartography`
- `sec-performing-cloud-forensics-investigation`
- `sec-performing-cloud-forensics-with-aws-cloudtrail`
- `sec-performing-cloud-incident-containment-procedures`
- `sec-performing-cloud-log-forensics-with-athena`
- `sec-performing-cloud-native-forensics-with-falco`
- `sec-performing-cloud-native-threat-hunting-with-aws-detective`
- `sec-performing-cloud-penetration-testing-with-pacu`
- `sec-performing-cloud-storage-forensic-acquisition`
- `sec-performing-container-escape-detection`
- `sec-performing-container-image-hardening`
- `sec-performing-container-security-scanning-with-trivy`
- `sec-performing-content-security-policy-bypass`
- `sec-performing-credential-access-with-lazagne`
- `sec-performing-cryptographic-audit-of-application`
- `sec-performing-csrf-attack-simulation`
- `sec-performing-cve-prioritization-with-kev-catalog`
- `sec-performing-dark-web-monitoring-for-threats`
- `sec-performing-deception-technology-deployment`
- `sec-performing-directory-traversal-testing`
- `sec-performing-disk-forensics-investigation`
- `sec-performing-dmarc-policy-enforcement-rollout`
- `sec-performing-dns-enumeration-and-zone-transfer`
- `sec-performing-dns-tunneling-detection`
- `sec-performing-docker-bench-security-assessment`
- `sec-performing-dynamic-analysis-of-android-app`
- `sec-performing-dynamic-analysis-with-any-run`
- `sec-performing-endpoint-forensics-investigation`
- `sec-performing-endpoint-vulnerability-remediation`
- `sec-performing-entitlement-review-with-sailpoint-iiq`
- `sec-performing-external-network-penetration-test`
- `sec-performing-false-positive-reduction-in-siem`
- `sec-performing-file-carving-with-foremost`
- `sec-performing-firmware-extraction-with-binwalk`
- `sec-performing-firmware-malware-analysis`
- `sec-performing-fuzzing-with-aflplusplus`
- `sec-performing-gcp-penetration-testing-with-gcpbucketbrute`
- `sec-performing-gcp-security-assessment-with-forseti`
- `sec-performing-graphql-depth-limit-attack`
- `sec-performing-graphql-introspection-attack`
- `sec-performing-graphql-security-assessment`
- `sec-performing-hardware-security-module-integration`
- `sec-performing-hash-cracking-with-hashcat`
- `sec-performing-http-parameter-pollution-attack`
- `sec-performing-ics-asset-discovery-with-claroty`
- `sec-performing-indicator-lifecycle-management`
- `sec-performing-initial-access-with-evilginx3`
- `sec-performing-insider-threat-investigation`
- `sec-performing-ioc-enrichment-automation`
- `sec-performing-ios-app-security-assessment`
- `sec-performing-iot-security-assessment`
- `sec-performing-ip-reputation-analysis-with-shodan`
- `sec-performing-jwt-none-algorithm-attack`
- `sec-performing-kerberoasting-attack`
- `sec-performing-kubernetes-cis-benchmark-with-kube-bench`
- `sec-performing-kubernetes-etcd-security-assessment`
- `sec-performing-kubernetes-penetration-testing`
- `sec-performing-lateral-movement-detection`
- `sec-performing-lateral-movement-with-wmiexec`
- `sec-performing-linux-log-forensics-investigation`
- `sec-performing-log-analysis-for-forensic-investigation`
- `sec-performing-log-source-onboarding-in-siem`
- `sec-performing-malware-hash-enrichment-with-virustotal`
- `sec-performing-malware-ioc-extraction`
- `sec-performing-malware-persistence-investigation`
- `sec-performing-malware-triage-with-yara`
- `sec-performing-memory-forensics-with-volatility3`
- `sec-performing-memory-forensics-with-volatility3-plugins`
- `sec-performing-mobile-app-certificate-pinning-bypass`
- `sec-performing-mobile-device-forensics-with-cellebrite`
- `sec-performing-network-forensics-with-wireshark`
- `sec-performing-network-packet-capture-analysis`
- `sec-performing-network-traffic-analysis-with-tshark`
- `sec-performing-network-traffic-analysis-with-zeek`
- `sec-performing-nist-csf-maturity-assessment`
- `sec-performing-oauth-scope-minimization-review`
- `sec-performing-oil-gas-cybersecurity-assessment`
- `sec-performing-open-source-intelligence-gathering`
- `sec-performing-osint-with-spiderfoot`
- `sec-performing-ot-network-security-assessment`
- `sec-performing-ot-vulnerability-assessment-with-claroty`
- `sec-performing-ot-vulnerability-scanning-safely`
- `sec-performing-packet-injection-attack`
- `sec-performing-paste-site-monitoring-for-credentials`
- `sec-performing-phishing-simulation-with-gophish`
- `sec-performing-physical-intrusion-assessment`
- `sec-performing-plc-firmware-security-analysis`
- `sec-performing-post-quantum-cryptography-migration`
- `sec-performing-power-grid-cybersecurity-assessment`
- `sec-performing-privacy-impact-assessment`
- `sec-performing-privilege-escalation-assessment`
- `sec-performing-privilege-escalation-on-linux`
- `sec-performing-privileged-account-access-review`
- `sec-performing-privileged-account-discovery`
- `sec-performing-purple-team-atomic-testing`
- `sec-performing-purple-team-exercise`
- `sec-performing-ransomware-response`
- `sec-performing-ransomware-tabletop-exercise`
- `sec-performing-red-team-phishing-with-gophish`
- `sec-performing-red-team-with-covenant`
- `sec-performing-s7comm-protocol-security-analysis`
- `sec-performing-sca-dependency-scanning-with-snyk`
- `sec-performing-scada-hmi-security-assessment`
- `sec-performing-second-order-sql-injection`
- `sec-performing-security-headers-audit`
- `sec-performing-serverless-function-security-review`
- `sec-performing-service-account-audit`
- `sec-performing-service-account-credential-rotation`
- `sec-performing-soap-web-service-security-testing`
- `sec-performing-soc-tabletop-exercise`
- `sec-performing-soc2-type2-audit-preparation`
- `sec-performing-sqlite-database-forensics`
- `sec-performing-ssl-certificate-lifecycle-management`
- `sec-performing-ssl-stripping-attack`
- `sec-performing-ssl-tls-inspection-configuration`
- `sec-performing-ssl-tls-security-assessment`
- `sec-performing-ssrf-vulnerability-exploitation`
- `sec-performing-static-malware-analysis-with-pe-studio`
- `sec-performing-steganography-detection`
- `sec-performing-subdomain-enumeration-with-subfinder`
- `sec-performing-supply-chain-attack-simulation`
- `sec-performing-thick-client-application-penetration-test`
- `sec-performing-threat-emulation-with-atomic-red-team`
- `sec-performing-threat-hunting-with-elastic-siem`
- `sec-performing-threat-hunting-with-yara-rules`
- `sec-performing-threat-intelligence-sharing-with-misp`
- `sec-performing-threat-landscape-assessment-for-sector`
- `sec-performing-threat-modeling-with-owasp-threat-dragon`
- `sec-performing-timeline-reconstruction-with-plaso`
- `sec-performing-user-behavior-analytics`
- `sec-performing-vlan-hopping-attack`
- `sec-performing-vulnerability-scanning-with-nessus`
- `sec-performing-web-application-firewall-bypass`
- `sec-performing-web-application-penetration-test`
- `sec-performing-web-application-scanning-with-nikto`
- `sec-performing-web-application-vulnerability-triage`
- `sec-performing-web-cache-deception-attack`
- `sec-performing-web-cache-poisoning-attack`
- `sec-performing-wifi-password-cracking-with-aircrack`
- `sec-performing-windows-artifact-analysis-with-eric-zimmerman-tools`
- `sec-performing-wireless-network-penetration-test`
- `sec-performing-wireless-security-assessment-with-kismet`
- `sec-performing-yara-rule-development-for-detection`

### Implementing (167)

- `sec-implementing-aes-encryption-for-data-at-rest`
- `sec-implementing-alert-fatigue-reduction`
- `sec-implementing-anti-phishing-training-program`
- `sec-implementing-anti-ransomware-group-policy`
- `sec-implementing-api-abuse-detection-with-rate-limiting`
- `sec-implementing-api-gateway-security-controls`
- `sec-implementing-api-key-security-controls`
- `sec-implementing-api-rate-limiting-and-throttling`
- `sec-implementing-api-schema-validation-security`
- `sec-implementing-api-security-posture-management`
- `sec-implementing-api-security-testing-with-42crunch`
- `sec-implementing-api-threat-protection-with-apigee`
- `sec-implementing-application-whitelisting-with-applocker`
- `sec-implementing-aqua-security-for-container-scanning`
- `sec-implementing-attack-path-analysis-with-xm-cyber`
- `sec-implementing-attack-surface-management`
- `sec-implementing-aws-config-rules-for-compliance`
- `sec-implementing-aws-iam-permission-boundaries`
- `sec-implementing-aws-macie-for-data-classification`
- `sec-implementing-aws-nitro-enclave-security`
- `sec-implementing-aws-security-hub`
- `sec-implementing-aws-security-hub-compliance`
- `sec-implementing-azure-ad-privileged-identity-management`
- `sec-implementing-azure-defender-for-cloud`
- `sec-implementing-beyondcorp-zero-trust-access-model`
- `sec-implementing-bgp-security-with-rpki`
- `sec-implementing-browser-isolation-for-zero-trust`
- `sec-implementing-canary-tokens-for-network-intrusion`
- `sec-implementing-cisa-zero-trust-maturity-model`
- `sec-implementing-cloud-dlp-for-data-protection`
- `sec-implementing-cloud-security-posture-management`
- `sec-implementing-cloud-trail-log-analysis`
- `sec-implementing-cloud-vulnerability-posture-management`
- `sec-implementing-cloud-waf-rules`
- `sec-implementing-cloud-workload-protection`
- `sec-implementing-code-signing-for-artifacts`
- `sec-implementing-conditional-access-policies-azure-ad`
- `sec-implementing-conduit-security-for-ot-remote-access`
- `sec-implementing-container-image-minimal-base-with-distroless`
- `sec-implementing-container-network-policies-with-calico`
- `sec-implementing-continuous-security-validation-with-bas`
- `sec-implementing-data-loss-prevention-with-microsoft-purview`
- `sec-implementing-ddos-mitigation-with-cloudflare`
- `sec-implementing-deception-based-detection-with-canarytoken`
- `sec-implementing-delinea-secret-server-for-pam`
- `sec-implementing-device-posture-assessment-in-zero-trust`
- `sec-implementing-devsecops-security-scanning`
- `sec-implementing-diamond-model-analysis`
- `sec-implementing-digital-signatures-with-ed25519`
- `sec-implementing-disk-encryption-with-bitlocker`
- `sec-implementing-dmarc-dkim-spf-email-security`
- `sec-implementing-dragos-platform-for-ot-monitoring`
- `sec-implementing-ebpf-security-monitoring`
- `sec-implementing-email-sandboxing-with-proofpoint`
- `sec-implementing-end-to-end-encryption-for-messaging`
- `sec-implementing-endpoint-detection-with-wazuh`
- `sec-implementing-endpoint-dlp-controls`
- `sec-implementing-envelope-encryption-with-aws-kms`
- `sec-implementing-epss-score-for-vulnerability-prioritization`
- `sec-implementing-file-integrity-monitoring-with-aide`
- `sec-implementing-fuzz-testing-in-cicd-with-aflplusplus`
- `sec-implementing-gcp-binary-authorization`
- `sec-implementing-gcp-organization-policy-constraints`
- `sec-implementing-gcp-vpc-firewall-rules`
- `sec-implementing-gdpr-data-protection-controls`
- `sec-implementing-gdpr-data-subject-access-request`
- `sec-implementing-github-advanced-security-for-code-scanning`
- `sec-implementing-google-workspace-admin-security`
- `sec-implementing-google-workspace-phishing-protection`
- `sec-implementing-google-workspace-sso-configuration`
- `sec-implementing-hardware-security-key-authentication`
- `sec-implementing-hashicorp-vault-dynamic-secrets`
- `sec-implementing-honeypot-for-ransomware-detection`
- `sec-implementing-honeytokens-for-breach-detection`
- `sec-implementing-ics-firewall-with-tofino`
- `sec-implementing-identity-governance-with-sailpoint`
- `sec-implementing-identity-verification-for-zero-trust`
- `sec-implementing-iec-62443-security-zones`
- `sec-implementing-image-provenance-verification-with-cosign`
- `sec-implementing-immutable-backup-with-restic`
- `sec-implementing-infrastructure-as-code-security-scanning`
- `sec-implementing-iso-27001-information-security-management`
- `sec-implementing-just-in-time-access-provisioning`
- `sec-implementing-jwt-signing-and-verification`
- `sec-implementing-kubernetes-network-policy-with-calico`
- `sec-implementing-kubernetes-pod-security-standards`
- `sec-implementing-llm-guardrails-for-security`
- `sec-implementing-log-forwarding-with-fluentd`
- `sec-implementing-log-integrity-with-blockchain`
- `sec-implementing-memory-protection-with-dep-aslr`
- `sec-implementing-microsegmentation-with-guardicore`
- `sec-implementing-mimecast-targeted-attack-protection`
- `sec-implementing-mitre-attack-coverage-mapping`
- `sec-implementing-mobile-application-management`
- `sec-implementing-mtls-for-zero-trust-services`
- `sec-implementing-nerc-cip-compliance-controls`
- `sec-implementing-network-access-control`
- `sec-implementing-network-access-control-with-cisco-ise`
- `sec-implementing-network-deception-with-honeypots`
- `sec-implementing-network-intrusion-prevention-with-suricata`
- `sec-implementing-network-policies-for-kubernetes`
- `sec-implementing-network-segmentation-for-ot`
- `sec-implementing-network-segmentation-with-firewall-zones`
- `sec-implementing-network-traffic-analysis-with-arkime`
- `sec-implementing-network-traffic-baselining`
- `sec-implementing-next-generation-firewall-with-palo-alto`
- `sec-implementing-opa-gatekeeper-for-policy-enforcement`
- `sec-implementing-ot-incident-response-playbook`
- `sec-implementing-ot-network-traffic-analysis-with-nozomi`
- `sec-implementing-pam-for-database-access`
- `sec-implementing-passwordless-auth-with-microsoft-entra`
- `sec-implementing-passwordless-authentication-with-fido2`
- `sec-implementing-patch-management-for-ot-systems`
- `sec-implementing-patch-management-workflow`
- `sec-implementing-pci-dss-compliance-controls`
- `sec-implementing-pod-security-admission-controller`
- `sec-implementing-policy-as-code-with-open-policy-agent`
- `sec-implementing-privileged-access-management-with-cyberark`
- `sec-implementing-privileged-access-workstation`
- `sec-implementing-privileged-session-monitoring`
- `sec-implementing-proofpoint-email-security-gateway`
- `sec-implementing-purdue-model-network-segmentation`
- `sec-implementing-ransomware-backup-strategy`
- `sec-implementing-ransomware-kill-switch-detection`
- `sec-implementing-rapid7-insightvm-for-scanning`
- `sec-implementing-rbac-hardening-for-kubernetes`
- `sec-implementing-rsa-key-pair-management`
- `sec-implementing-runtime-application-self-protection`
- `sec-implementing-runtime-security-with-tetragon`
- `sec-implementing-saml-sso-with-okta`
- `sec-implementing-scim-provisioning-with-okta`
- `sec-implementing-secret-scanning-with-gitleaks`
- `sec-implementing-secrets-management-with-vault`
- `sec-implementing-secrets-scanning-in-ci-cd`
- `sec-implementing-security-chaos-engineering`
- `sec-implementing-security-information-sharing-with-stix2`
- `sec-implementing-security-monitoring-with-datadog`
- `sec-implementing-semgrep-for-custom-sast-rules`
- `sec-implementing-siem-correlation-rules-for-apt`
- `sec-implementing-siem-use-case-tuning`
- `sec-implementing-siem-use-cases-for-detection`
- `sec-implementing-sigstore-for-software-signing`
- `sec-implementing-soar-automation-with-phantom`
- `sec-implementing-soar-playbook-for-phishing`
- `sec-implementing-soar-playbook-with-palo-alto-xsoar`
- `sec-implementing-stix-taxii-feed-integration`
- `sec-implementing-supply-chain-security-with-in-toto`
- `sec-implementing-syslog-centralization-with-rsyslog`
- `sec-implementing-taxii-server-with-opentaxii`
- `sec-implementing-threat-intelligence-lifecycle-management`
- `sec-implementing-threat-modeling-with-mitre-attack`
- `sec-implementing-ticketing-system-for-incidents`
- `sec-implementing-usb-device-control-policy`
- `sec-implementing-velociraptor-for-ir-collection`
- `sec-implementing-vulnerability-management-with-greenbone`
- `sec-implementing-vulnerability-remediation-sla`
- `sec-implementing-vulnerability-sla-breach-alerting`
- `sec-implementing-web-application-logging-with-modsecurity`
- `sec-implementing-zero-knowledge-proof-for-authentication`
- `sec-implementing-zero-standing-privilege-with-cyberark`
- `sec-implementing-zero-trust-dns-with-nextdns`
- `sec-implementing-zero-trust-for-saas-applications`
- `sec-implementing-zero-trust-in-cloud`
- `sec-implementing-zero-trust-network-access`
- `sec-implementing-zero-trust-network-access-with-zscaler`
- `sec-implementing-zero-trust-with-beyondcorp`
- `sec-implementing-zero-trust-with-hashicorp-boundary`

### Detecting (87)

- `sec-detecting-ai-model-prompt-injection-attacks`
- `sec-detecting-anomalies-in-industrial-control-systems`
- `sec-detecting-anomalous-authentication-patterns`
- `sec-detecting-api-enumeration-attacks`
- `sec-detecting-arp-poisoning-in-network-traffic`
- `sec-detecting-attacks-on-historian-servers`
- `sec-detecting-attacks-on-scada-systems`
- `sec-detecting-aws-cloudtrail-anomalies`
- `sec-detecting-aws-credential-exposure-with-trufflehog`
- `sec-detecting-aws-guardduty-findings-automation`
- `sec-detecting-aws-iam-privilege-escalation`
- `sec-detecting-azure-lateral-movement`
- `sec-detecting-azure-service-principal-abuse`
- `sec-detecting-azure-storage-account-misconfigurations`
- `sec-detecting-beaconing-patterns-with-zeek`
- `sec-detecting-bluetooth-low-energy-attacks`
- `sec-detecting-broken-object-property-level-authorization`
- `sec-detecting-business-email-compromise`
- `sec-detecting-business-email-compromise-with-ai`
- `sec-detecting-cloud-threats-with-guardduty`
- `sec-detecting-command-and-control-over-dns`
- `sec-detecting-compromised-cloud-credentials`
- `sec-detecting-container-drift-at-runtime`
- `sec-detecting-container-escape-attempts`
- `sec-detecting-container-escape-with-falco-rules`
- `sec-detecting-credential-dumping-techniques`
- `sec-detecting-cryptomining-in-cloud`
- `sec-detecting-dcsync-attack-in-active-directory`
- `sec-detecting-deepfake-audio-in-vishing-attacks`
- `sec-detecting-dll-sideloading-attacks`
- `sec-detecting-dnp3-protocol-anomalies`
- `sec-detecting-dns-exfiltration-with-dns-query-analysis`
- `sec-detecting-email-account-compromise`
- `sec-detecting-email-forwarding-rules-attack`
- `sec-detecting-evasion-techniques-in-endpoint-logs`
- `sec-detecting-exfiltration-over-dns-with-zeek`
- `sec-detecting-fileless-attacks-on-endpoints`
- `sec-detecting-fileless-malware-techniques`
- `sec-detecting-golden-ticket-attacks-in-kerberos-logs`
- `sec-detecting-golden-ticket-forgery`
- `sec-detecting-insider-data-exfiltration-via-dlp`
- `sec-detecting-insider-threat-behaviors`
- `sec-detecting-insider-threat-with-ueba`
- `sec-detecting-kerberoasting-attacks`
- `sec-detecting-lateral-movement-in-network`
- `sec-detecting-lateral-movement-with-splunk`
- `sec-detecting-lateral-movement-with-zeek`
- `sec-detecting-living-off-the-land-attacks`
- `sec-detecting-living-off-the-land-with-lolbas`
- `sec-detecting-malicious-scheduled-tasks-with-sysmon`
- `sec-detecting-mimikatz-execution-patterns`
- `sec-detecting-misconfigured-azure-storage`
- `sec-detecting-mobile-malware-behavior`
- `sec-detecting-modbus-command-injection-attacks`
- `sec-detecting-modbus-protocol-anomalies`
- `sec-detecting-network-anomalies-with-zeek`
- `sec-detecting-network-scanning-with-ids-signatures`
- `sec-detecting-ntlm-relay-with-event-correlation`
- `sec-detecting-oauth-token-theft`
- `sec-detecting-pass-the-hash-attacks`
- `sec-detecting-pass-the-ticket-attacks`
- `sec-detecting-port-scanning-with-fail2ban`
- `sec-detecting-privilege-escalation-attempts`
- `sec-detecting-privilege-escalation-in-kubernetes-pods`
- `sec-detecting-process-hollowing-technique`
- `sec-detecting-process-injection-techniques`
- `sec-detecting-qr-code-phishing-with-email-security`
- `sec-detecting-ransomware-encryption-behavior`
- `sec-detecting-ransomware-precursors-in-network`
- `sec-detecting-rdp-brute-force-attacks`
- `sec-detecting-rootkit-activity`
- `sec-detecting-s3-data-exfiltration-attempts`
- `sec-detecting-serverless-function-injection`
- `sec-detecting-service-account-abuse`
- `sec-detecting-shadow-api-endpoints`
- `sec-detecting-shadow-it-cloud-usage`
- `sec-detecting-spearphishing-with-email-gateway`
- `sec-detecting-sql-injection-via-waf-logs`
- `sec-detecting-stuxnet-style-attacks`
- `sec-detecting-supply-chain-attacks-in-ci-cd`
- `sec-detecting-suspicious-oauth-application-consent`
- `sec-detecting-suspicious-powershell-execution`
- `sec-detecting-t1003-credential-dumping-with-edr`
- `sec-detecting-t1055-process-injection-with-sysmon`
- `sec-detecting-t1548-abuse-elevation-control-mechanism`
- `sec-detecting-typosquatting-packages-in-npm-pypi`
- `sec-detecting-wmi-persistence`

### Analyzing (76)

- `sec-analyzing-active-directory-acl-abuse`
- `sec-analyzing-android-malware-with-apktool`
- `sec-analyzing-api-gateway-access-logs`
- `sec-analyzing-apt-group-with-mitre-navigator`
- `sec-analyzing-azure-activity-logs-for-threats`
- `sec-analyzing-bootkit-and-rootkit-samples`
- `sec-analyzing-browser-forensics-with-hindsight`
- `sec-analyzing-campaign-attribution-evidence`
- `sec-analyzing-certificate-transparency-for-phishing`
- `sec-analyzing-cloud-storage-access-patterns`
- `sec-analyzing-cobalt-strike-beacon-configuration`
- `sec-analyzing-cobaltstrike-malleable-c2-profiles`
- `sec-analyzing-command-and-control-communication`
- `sec-analyzing-cyber-kill-chain`
- `sec-analyzing-disk-image-with-autopsy`
- `sec-analyzing-dns-logs-for-exfiltration`
- `sec-analyzing-docker-container-forensics`
- `sec-analyzing-email-headers-for-phishing-investigation`
- `sec-analyzing-ethereum-smart-contract-vulnerabilities`
- `sec-analyzing-golang-malware-with-ghidra`
- `sec-analyzing-heap-spray-exploitation`
- `sec-analyzing-indicators-of-compromise`
- `sec-analyzing-ios-app-security-with-objection`
- `sec-analyzing-kubernetes-audit-logs`
- `sec-analyzing-linux-audit-logs-for-intrusion`
- `sec-analyzing-linux-elf-malware`
- `sec-analyzing-linux-kernel-rootkits`
- `sec-analyzing-linux-system-artifacts`
- `sec-analyzing-lnk-file-and-jump-list-artifacts`
- `sec-analyzing-macro-malware-in-office-documents`
- `sec-analyzing-malicious-pdf-with-peepdf`
- `sec-analyzing-malicious-url-with-urlscan`
- `sec-analyzing-malware-behavior-with-cuckoo-sandbox`
- `sec-analyzing-malware-family-relationships-with-malpedia`
- `sec-analyzing-malware-persistence-with-autoruns`
- `sec-analyzing-malware-sandbox-evasion-techniques`
- `sec-analyzing-memory-dumps-with-volatility`
- `sec-analyzing-memory-forensics-with-lime-and-volatility`
- `sec-analyzing-mft-for-deleted-file-recovery`
- `sec-analyzing-network-covert-channels-in-malware`
- `sec-analyzing-network-flow-data-with-netflow`
- `sec-analyzing-network-packets-with-scapy`
- `sec-analyzing-network-traffic-for-incidents`
- `sec-analyzing-network-traffic-of-malware`
- `sec-analyzing-network-traffic-with-wireshark`
- `sec-analyzing-office365-audit-logs-for-compromise`
- `sec-analyzing-outlook-pst-for-email-forensics`
- `sec-analyzing-packed-malware-with-upx-unpacker`
- `sec-analyzing-pdf-malware-with-pdfid`
- `sec-analyzing-persistence-mechanisms-in-linux`
- `sec-analyzing-powershell-empire-artifacts`
- `sec-analyzing-powershell-script-block-logging`
- `sec-analyzing-prefetch-files-for-execution-history`
- `sec-analyzing-ransomware-encryption-mechanisms`
- `sec-analyzing-ransomware-leak-site-intelligence`
- `sec-analyzing-ransomware-network-indicators`
- `sec-analyzing-ransomware-payment-wallets`
- `sec-analyzing-sbom-for-supply-chain-vulnerabilities`
- `sec-analyzing-security-logs-with-splunk`
- `sec-analyzing-slack-space-and-file-system-artifacts`
- `sec-analyzing-supply-chain-malware-artifacts`
- `sec-analyzing-threat-actor-ttps-with-mitre-attack`
- `sec-analyzing-threat-actor-ttps-with-mitre-navigator`
- `sec-analyzing-threat-intelligence-feeds`
- `sec-analyzing-threat-landscape-with-misp`
- `sec-analyzing-tls-certificate-transparency-logs`
- `sec-analyzing-typosquatting-domains-with-dnstwist`
- `sec-analyzing-uefi-bootkit-persistence`
- `sec-analyzing-usb-device-connection-history`
- `sec-analyzing-web-server-logs-for-intrusion`
- `sec-analyzing-windows-amcache-artifacts`
- `sec-analyzing-windows-event-logs-in-splunk`
- `sec-analyzing-windows-lnk-files-for-artifacts`
- `sec-analyzing-windows-prefetch-with-python`
- `sec-analyzing-windows-registry-for-artifacts`
- `sec-analyzing-windows-shellbag-artifacts`

### Building (34)

- `sec-building-adversary-infrastructure-tracking-system`
- `sec-building-attack-pattern-library-from-cti-reports`
- `sec-building-automated-malware-submission-pipeline`
- `sec-building-c2-infrastructure-with-sliver-framework`
- `sec-building-cloud-siem-with-sentinel`
- `sec-building-detection-rule-with-splunk-spl`
- `sec-building-detection-rules-with-sigma`
- `sec-building-devsecops-pipeline-with-gitlab-ci`
- `sec-building-identity-federation-with-saml-azure-ad`
- `sec-building-identity-governance-lifecycle-process`
- `sec-building-incident-response-dashboard`
- `sec-building-incident-response-playbook`
- `sec-building-incident-timeline-with-timesketch`
- `sec-building-ioc-defanging-and-sharing-pipeline`
- `sec-building-ioc-enrichment-pipeline-with-opencti`
- `sec-building-malware-incident-communication-template`
- `sec-building-patch-tuesday-response-process`
- `sec-building-phishing-reporting-button-workflow`
- `sec-building-ransomware-playbook-with-cisa-framework`
- `sec-building-red-team-c2-infrastructure-with-havoc`
- `sec-building-role-mining-for-rbac-optimization`
- `sec-building-soc-escalation-matrix`
- `sec-building-soc-metrics-and-kpi-tracking`
- `sec-building-soc-playbook-for-ransomware`
- `sec-building-threat-actor-profile-from-osint`
- `sec-building-threat-feed-aggregation-with-misp`
- `sec-building-threat-hunt-hypothesis-framework`
- `sec-building-threat-intelligence-enrichment-in-splunk`
- `sec-building-threat-intelligence-feed-integration`
- `sec-building-threat-intelligence-platform`
- `sec-building-vulnerability-aging-and-sla-tracking`
- `sec-building-vulnerability-dashboard-with-defectdojo`
- `sec-building-vulnerability-exception-tracking-system`
- `sec-building-vulnerability-scanning-workflow`

### Hunting (34)

- `sec-hunting-advanced-persistent-threats`
- `sec-hunting-credential-stuffing-attacks`
- `sec-hunting-for-anomalous-powershell-execution`
- `sec-hunting-for-beaconing-with-frequency-analysis`
- `sec-hunting-for-cobalt-strike-beacons`
- `sec-hunting-for-command-and-control-beaconing`
- `sec-hunting-for-data-exfiltration-indicators`
- `sec-hunting-for-data-staging-before-exfiltration`
- `sec-hunting-for-dcom-lateral-movement`
- `sec-hunting-for-dcsync-attacks`
- `sec-hunting-for-defense-evasion-via-timestomping`
- `sec-hunting-for-dns-based-persistence`
- `sec-hunting-for-dns-tunneling-with-zeek`
- `sec-hunting-for-domain-fronting-c2-traffic`
- `sec-hunting-for-lateral-movement-via-wmi`
- `sec-hunting-for-living-off-the-cloud-techniques`
- `sec-hunting-for-living-off-the-land-binaries`
- `sec-hunting-for-lolbins-execution-in-endpoint-logs`
- `sec-hunting-for-ntlm-relay-attacks`
- `sec-hunting-for-persistence-mechanisms-in-windows`
- `sec-hunting-for-persistence-via-wmi-subscriptions`
- `sec-hunting-for-process-injection-techniques`
- `sec-hunting-for-registry-persistence-mechanisms`
- `sec-hunting-for-registry-run-key-persistence`
- `sec-hunting-for-scheduled-task-persistence`
- `sec-hunting-for-shadow-copy-deletion`
- `sec-hunting-for-spearphishing-indicators`
- `sec-hunting-for-startup-folder-persistence`
- `sec-hunting-for-supply-chain-compromise`
- `sec-hunting-for-suspicious-scheduled-tasks`
- `sec-hunting-for-t1098-account-manipulation`
- `sec-hunting-for-unusual-network-connections`
- `sec-hunting-for-unusual-service-installations`
- `sec-hunting-for-webshell-activity`

### Exploiting (32)

- `sec-exploiting-active-directory-certificate-services-esc1`
- `sec-exploiting-active-directory-with-bloodhound`
- `sec-exploiting-api-injection-vulnerabilities`
- `sec-exploiting-bgp-hijacking-vulnerabilities`
- `sec-exploiting-broken-function-level-authorization`
- `sec-exploiting-broken-link-hijacking`
- `sec-exploiting-constrained-delegation-abuse`
- `sec-exploiting-deeplink-vulnerabilities`
- `sec-exploiting-excessive-data-exposure-in-api`
- `sec-exploiting-http-request-smuggling`
- `sec-exploiting-idor-vulnerabilities`
- `sec-exploiting-insecure-data-storage-in-mobile`
- `sec-exploiting-insecure-deserialization`
- `sec-exploiting-ipv6-vulnerabilities`
- `sec-exploiting-jwt-algorithm-confusion-attack`
- `sec-exploiting-kerberoasting-with-impacket`
- `sec-exploiting-mass-assignment-in-rest-apis`
- `sec-exploiting-ms17-010-eternalblue-vulnerability`
- `sec-exploiting-nopac-cve-2021-42278-42287`
- `sec-exploiting-nosql-injection-vulnerabilities`
- `sec-exploiting-oauth-misconfiguration`
- `sec-exploiting-prototype-pollution-in-javascript`
- `sec-exploiting-race-condition-vulnerabilities`
- `sec-exploiting-server-side-request-forgery`
- `sec-exploiting-smb-vulnerabilities-with-metasploit`
- `sec-exploiting-sql-injection-vulnerabilities`
- `sec-exploiting-sql-injection-with-sqlmap`
- `sec-exploiting-template-injection-vulnerabilities`
- `sec-exploiting-type-juggling-vulnerabilities`
- `sec-exploiting-vulnerabilities-with-metasploit-framework`
- `sec-exploiting-websocket-vulnerabilities`
- `sec-exploiting-zerologon-vulnerability-cve-2020-1472`

### Testing (22)

- `sec-testing-android-intents-for-vulnerabilities`
- `sec-testing-api-authentication-weaknesses`
- `sec-testing-api-for-broken-object-level-authorization`
- `sec-testing-api-for-mass-assignment-vulnerability`
- `sec-testing-api-security-with-owasp-top-10`
- `sec-testing-cors-misconfiguration`
- `sec-testing-for-broken-access-control`
- `sec-testing-for-business-logic-vulnerabilities`
- `sec-testing-for-email-header-injection`
- `sec-testing-for-host-header-injection`
- `sec-testing-for-json-web-token-vulnerabilities`
- `sec-testing-for-open-redirect-vulnerabilities`
- `sec-testing-for-sensitive-data-exposure`
- `sec-testing-for-xml-injection-vulnerabilities`
- `sec-testing-for-xss-vulnerabilities`
- `sec-testing-for-xss-vulnerabilities-with-burpsuite`
- `sec-testing-for-xxe-injection-vulnerabilities`
- `sec-testing-jwt-token-security`
- `sec-testing-mobile-api-authentication`
- `sec-testing-oauth2-implementation-flaws`
- `sec-testing-ransomware-recovery-procedures`
- `sec-testing-websocket-api-security`

### Conducting (20)

- `sec-conducting-api-security-testing`
- `sec-conducting-cloud-incident-response`
- `sec-conducting-cloud-penetration-testing`
- `sec-conducting-domain-persistence-with-dcsync`
- `sec-conducting-external-reconnaissance-with-osint`
- `sec-conducting-full-scope-red-team-engagement`
- `sec-conducting-internal-network-penetration-test`
- `sec-conducting-internal-reconnaissance-with-bloodhound-ce`
- `sec-conducting-malware-incident-response`
- `sec-conducting-man-in-the-middle-attack-simulation`
- `sec-conducting-memory-forensics-with-volatility`
- `sec-conducting-mobile-app-penetration-test`
- `sec-conducting-network-penetration-test`
- `sec-conducting-pass-the-ticket-attack`
- `sec-conducting-phishing-incident-response`
- `sec-conducting-post-incident-lessons-learned`
- `sec-conducting-social-engineering-penetration-test`
- `sec-conducting-social-engineering-pretext-call`
- `sec-conducting-spearphishing-simulation-campaign`
- `sec-conducting-wireless-network-penetration-test`

### Configuring (18)

- `sec-configuring-active-directory-tiered-model`
- `sec-configuring-aws-verified-access-for-ztna`
- `sec-configuring-certificate-authority-with-openssl`
- `sec-configuring-host-based-intrusion-detection`
- `sec-configuring-hsm-for-key-storage`
- `sec-configuring-identity-aware-proxy-with-google-iap`
- `sec-configuring-ldap-security-hardening`
- `sec-configuring-microsegmentation-for-zero-trust`
- `sec-configuring-multi-factor-authentication-with-duo`
- `sec-configuring-network-segmentation-with-vlans`
- `sec-configuring-oauth2-authorization-flow`
- `sec-configuring-pfsense-firewall-rules`
- `sec-configuring-snort-ids-for-intrusion-detection`
- `sec-configuring-suricata-for-network-monitoring`
- `sec-configuring-tls-1-3-for-secure-communications`
- `sec-configuring-windows-defender-advanced-settings`
- `sec-configuring-windows-event-logging-for-detection`
- `sec-configuring-zscaler-private-access-for-ztna`

### Securing (12)

- `sec-securing-api-gateway-with-aws-waf`
- `sec-securing-aws-iam-permissions`
- `sec-securing-aws-lambda-execution-roles`
- `sec-securing-azure-with-microsoft-defender`
- `sec-securing-container-registry-images`
- `sec-securing-container-registry-with-harbor`
- `sec-securing-github-actions-workflows`
- `sec-securing-helm-chart-deployments`
- `sec-securing-historian-server-in-ot-environment`
- `sec-securing-kubernetes-on-cloud`
- `sec-securing-remote-access-to-ot-environment`
- `sec-securing-serverless-functions`

### Deploying (9)

- `sec-deploying-active-directory-honeytokens`
- `sec-deploying-cloudflare-access-for-zero-trust`
- `sec-deploying-decoy-files-for-ransomware-detection`
- `sec-deploying-edr-agent-with-crowdstrike`
- `sec-deploying-osquery-for-endpoint-monitoring`
- `sec-deploying-palo-alto-prisma-access-zero-trust`
- `sec-deploying-ransomware-canary-files`
- `sec-deploying-software-defined-perimeter`
- `sec-deploying-tailscale-for-zero-trust-vpn`

### Auditing (7)

- `sec-auditing-aws-s3-bucket-permissions`
- `sec-auditing-azure-active-directory-configuration`
- `sec-auditing-cloud-with-cis-benchmarks`
- `sec-auditing-gcp-iam-permissions`
- `sec-auditing-kubernetes-cluster-rbac`
- `sec-auditing-terraform-infrastructure-for-security`
- `sec-auditing-tls-certificate-transparency-logs`

### Extracting (6)

- `sec-extracting-browser-history-artifacts`
- `sec-extracting-config-from-agent-tesla-rat`
- `sec-extracting-credentials-from-memory-dump`
- `sec-extracting-iocs-from-malware-samples`
- `sec-extracting-memory-artifacts-with-rekall`
- `sec-extracting-windows-event-logs-artifacts`

### Reverse (6)

- `sec-reverse-engineering-android-malware-with-jadx`
- `sec-reverse-engineering-dotnet-malware-with-dnspy`
- `sec-reverse-engineering-ios-app-with-frida`
- `sec-reverse-engineering-malware-with-ghidra`
- `sec-reverse-engineering-ransomware-encryption-routine`
- `sec-reverse-engineering-rust-malware`

### Scanning (6)

- `sec-scanning-container-images-with-grype`
- `sec-scanning-containers-with-trivy-in-cicd`
- `sec-scanning-docker-images-with-trivy`
- `sec-scanning-infrastructure-with-nessus`
- `sec-scanning-kubernetes-manifests-with-kubesec`
- `sec-scanning-network-with-nmap-advanced`

### Collecting (4)

- `sec-collecting-indicators-of-compromise`
- `sec-collecting-open-source-intelligence`
- `sec-collecting-threat-intelligence-with-misp`
- `sec-collecting-volatile-evidence-from-compromised-host`

### Executing (4)

- `sec-executing-active-directory-attack-simulation`
- `sec-executing-phishing-simulation-campaign`
- `sec-executing-red-team-engagement-planning`
- `sec-executing-red-team-exercise`

### Hardening (4)

- `sec-hardening-docker-containers-for-production`
- `sec-hardening-docker-daemon-configuration`
- `sec-hardening-linux-endpoint-with-cis-benchmark`
- `sec-hardening-windows-endpoint-with-cis-benchmark`

### Triaging (4)

- `sec-triaging-security-alerts-in-splunk`
- `sec-triaging-security-incident`
- `sec-triaging-security-incident-with-ir-playbook`
- `sec-triaging-vulnerabilities-with-ssvc-framework`

### Investigating (3)

- `sec-investigating-insider-threat-indicators`
- `sec-investigating-phishing-email-incident`
- `sec-investigating-ransomware-attack-artifacts`

### Diğer (27)

- `sec-acquiring-disk-image-with-dd-and-dcfldd`
- `sec-automating-ioc-enrichment`
- `sec-bypassing-authentication-with-forced-browsing`
- `sec-containing-active-breach`
- `sec-correlating-security-events-in-qradar`
- `sec-correlating-threat-campaigns`
- `sec-deobfuscating-javascript-malware`
- `sec-deobfuscating-powershell-obfuscated-malware`
- `sec-eradicating-malware-from-infected-systems`
- `sec-evaluating-threat-intelligence-platforms`
- `sec-generating-threat-intelligence-reports`
- `sec-integrating-dast-with-owasp-zap-in-pipeline`
- `sec-integrating-sast-into-github-actions-pipeline`
- `sec-intercepting-mobile-traffic-with-burpsuite`
- `sec-managing-cloud-identity-with-okta`
- `sec-managing-intelligence-lifecycle`
- `sec-mapping-mitre-attack-techniques`
- `sec-monitoring-darkweb-sources`
- `sec-monitoring-scada-modbus-traffic-anomalies`
- `sec-prioritizing-vulnerabilities-with-cvss-scoring`
- `sec-processing-stix-taxii-feeds`
- `sec-profiling-threat-actor-groups`
- `sec-recovering-deleted-files-with-photorec`
- `sec-recovering-from-ransomware-attack`
- `sec-remediating-s3-bucket-misconfiguration`
- `sec-tracking-threat-actor-infrastructure`
- `sec-validating-backup-integrity-for-recovery`

---

## mutfak-consulting

> İş/ops/C-level danışmanlık (ali-*) · **347 skill**

### Senior (14)

- `ali-senior-architect`
- `ali-senior-backend`
- `ali-senior-computer-vision`
- `ali-senior-data-engineer`
- `ali-senior-data-scientist`
- `ali-senior-devops`
- `ali-senior-frontend`
- `ali-senior-fullstack`
- `ali-senior-ml-engineer`
- `ali-senior-pm`
- `ali-senior-prompt-engineer`
- `ali-senior-qa`
- `ali-senior-secops`
- `ali-senior-security`

### Engineering (12)

- `ali-engineering-advanced-skills`
- `ali-engineering-chaos-engineering`
- `ali-engineering-feature-flags-architect`
- `ali-engineering-handoff`
- `ali-engineering-kubernetes-operator`
- `ali-engineering-run`
- `ali-engineering-skills`
- `ali-engineering-slo-architect`
- `ali-engineering-status`
- `ali-engineering-team-init`
- `ali-engineering-team-review`
- `ali-engineering-team-status`

### C (7)

- `ali-c-level-advisor-chief-ai-officer-advisor`
- `ali-c-level-advisor-chief-customer-officer-advisor`
- `ali-c-level-advisor-chief-data-officer-advisor`
- `ali-c-level-advisor-general-counsel-advisor`
- `ali-c-level-advisor-vpe-advisor`
- `ali-c-level-agents`
- `ali-c-level-skills`

### Marketing (7)

- `ali-marketing-context`
- `ali-marketing-demand-acquisition`
- `ali-marketing-ideas`
- `ali-marketing-ops`
- `ali-marketing-psychology`
- `ali-marketing-skills`
- `ali-marketing-strategy-pmm`

### Product (6)

- `ali-product-analytics`
- `ali-product-discovery`
- `ali-product-manager-toolkit`
- `ali-product-research`
- `ali-product-skills`
- `ali-product-strategist`

### Board (4)

- `ali-board`
- `ali-board-deck-builder`
- `ali-board-meeting`
- `ali-board-prep`

### Chief (4)

- `ali-chief-ai-officer-advisor`
- `ali-chief-customer-officer-advisor`
- `ali-chief-data-officer-advisor`
- `ali-chief-of-staff`

### Content (4)

- `ali-content-creator`
- `ali-content-humanizer`
- `ali-content-production`
- `ali-content-strategy`

### Research (4)

- `ali-research`
- `ali-research-finance`
- `ali-research-ops-skills`
- `ali-research-summarizer`

### Agent (3)

- `ali-agent-designer`
- `ali-agent-protocol`
- `ali-agent-workflow-designer`

### Ai (3)

- `ali-ai-act-readiness`
- `ali-ai-security`
- `ali-ai-seo`

### Business (3)

- `ali-business-growth-skills`
- `ali-business-investment-advisor`
- `ali-business-operations-skills`

### Code (3)

- `ali-code-reviewer`
- `ali-code-to-prd`
- `ali-code-tour`

### Commercial (3)

- `ali-commercial-forecaster`
- `ali-commercial-policy`
- `ali-commercial-skills`

### Md (3)

- `ali-md-document`
- `ali-md-review`
- `ali-md-slides`

### Quality (3)

- `ali-quality-documentation-manager`
- `ali-quality-manager-qmr`
- `ali-quality-manager-qms-iso13485`

### Ra (3)

- `ali-ra-qm-skills`
- `ali-ra-qm-team-eu-ai-act-specialist`
- `ali-ra-qm-team-iso42001-specialist`

### Social (3)

- `ali-social-content`
- `ali-social-media-analyzer`
- `ali-social-media-manager`

### Diğer (258)

- `ali-a11y-audit`
- `ali-ab-test-setup`
- `ali-ad-creative`
- `ali-adversarial-reviewer`
- `ali-aeo`
- `ali-agenthub`
- `ali-agile-product-owner`
- `ali-aims-audit`
- `ali-analytics-tracking`
- `ali-andreessen`
- `ali-api-design-reviewer`
- `ali-api-test-suite-builder`
- `ali-app-store-optimization`
- `ali-apple-hig-expert`
- `ali-atlassian-admin`
- `ali-atlassian-templates`
- `ali-autoresearch-agent`
- `ali-aws-solution-architect`
- `ali-azure-cloud-architect`
- `ali-behuman`
- `ali-boardroom`
- `ali-brand-guidelines`
- `ali-brief`
- `ali-browser-automation`
- `ali-browserstack`
- `ali-caio-review`
- `ali-campaign-analytics`
- `ali-capa-officer`
- `ali-capacity-planner`
- `ali-capture`
- `ali-caveman`
- `ali-cco-review`
- `ali-cdo-review`
- `ali-ceo-advisor`
- `ali-cfo-advisor`
- `ali-cfo-review`
- `ali-challenge`
- `ali-change-management`
- `ali-changelog-generator`
- `ali-channel-economics`
- `ali-chaos-engineering`
- `ali-chro-advisor`
- `ali-churn-prevention`
- `ali-ci-cd-pipeline-builder`
- `ali-ciso-advisor`
- `ali-ciso-review`
- `ali-claude-coach`
- `ali-clinical-research`
- `ali-cloud-security`
- `ali-cmo-advisor`
- `ali-cmo-review`
- `ali-codebase-onboarding`
- `ali-cold-email`
- `ali-command-guide`
- `ali-company-os`
- `ali-competitive-intel`
- `ali-competitive-teardown`
- `ali-competitor-alternatives`
- `ali-compliance-os`
- `ali-compliance-readiness`
- `ali-confluence-expert`
- `ali-context-engine`
- `ali-contract-and-proposal-writer`
- `ali-coo-advisor`
- `ali-copy-editing`
- `ali-copywriting`
- `ali-coverage`
- `ali-cpo-advisor`
- `ali-cpo-review`
- `ali-cro-advisor`
- `ali-cro-review`
- `ali-cross-eval`
- `ali-cs-onboard`
- `ali-cto-advisor`
- `ali-cto-review`
- `ali-culture-architect`
- `ali-customer-success-manager`
- `ali-data-quality-auditor`
- `ali-database-designer`
- `ali-database-schema-designer`
- `ali-deal-desk`
- `ali-decide`
- `ali-decision-logger`
- `ali-demo-video`
- `ali-dependency-auditor`
- `ali-design-system`
- `ali-docker-development`
- `ali-dossier`
- `ali-email-sequence`
- `ali-email-template-builder`
- `ali-env-secrets-manager`
- `ali-epic-design`
- `ali-eu-ai-act-specialist`
- `ali-eval`
- `ali-execute`
- `ali-executive-mentor`
- `ali-experiment-designer`
- `ali-extract`
- `ali-fda-consultant-specialist`
- `ali-fda-qsr-audit-prep`
- `ali-feature-flags-architect`
- `ali-finance-skills`
- `ali-financial-analyst`
- `ali-fix`
- `ali-focused-fix`
- `ali-form-cro`
- `ali-founder-coach`
- `ali-founder-mode`
- `ali-free-tool-strategy`
- `ali-freeze`
- `ali-full-page-screenshot`
- `ali-gc-review`
- `ali-gcp-cloud-architect`
- `ali-gdpr-audit-prep`
- `ali-gdpr-dsgvo-expert`
- `ali-general-counsel-advisor`
- `ali-generate`
- `ali-git-worktree-manager`
- `ali-google-workspace-cli`
- `ali-grants`
- `ali-grill-me`
- `ali-grill-with-docs`
- `ali-handoff`
- `ali-hard-call`
- `ali-helm-chart-builder`
- `ali-inbox-setup`
- `ali-inbox-triage`
- `ali-incident-commander`
- `ali-incident-response`
- `ali-information-security-manager-iso27001`
- `ali-init`
- `ali-internal-comms`
- `ali-internal-narrative`
- `ali-interview-system-designer`
- `ali-intl-expansion`
- `ali-isms-audit-expert`
- `ali-iso13485-audit-prep`
- `ali-iso27001-audit-prep`
- `ali-iso42001-specialist`
- `ali-jira-expert`
- `ali-karpathy-coder`
- `ali-knowledge-ops`
- `ali-kubernetes-operator`
- `ali-landing`
- `ali-landing-page-generator`
- `ali-launch-strategy`
- `ali-litreview`
- `ali-llm-cost-optimizer`
- `ali-llm-wiki`
- `ali-loop`
- `ali-ma-playbook`
- `ali-markdown-html-orchestrator`
- `ali-market-research`
- `ali-mcp-server-builder`
- `ali-mdr-745-specialist`
- `ali-meeting-analyzer`
- `ali-merge`
- `ali-migrate`
- `ali-migration-architect`
- `ali-monorepo-navigator`
- `ali-ms365-tenant-manager`
- `ali-notebooklm`
- `ali-observability-designer`
- `ali-office-hours`
- `ali-onboard`
- `ali-onboarding-cro`
- `ali-org-health-diagnostic`
- `ali-page-cro`
- `ali-paid-ads`
- `ali-partnerships-architect`
- `ali-patent`
- `ali-paywall-upgrade-cro`
- `ali-performance-profiler`
- `ali-pm-skills`
- `ali-popup-cro`
- `ali-post-mortem`
- `ali-postmortem`
- `ali-pr-review-expert`
- `ali-pricing-strategist`
- `ali-pricing-strategy`
- `ali-process-mapper`
- `ali-procurement-optimizer`
- `ali-programmatic-seo`
- `ali-promote`
- `ali-prompt-engineer-toolkit`
- `ali-prompt-governance`
- `ali-pulse`
- `ali-pw`
- `ali-qms-audit-expert`
- `ali-rag-architect`
- `ali-red-team`
- `ali-referral-program`
- `ali-reflect`
- `ali-regulatory-affairs-head`
- `ali-release-manager`
- `ali-remember`
- `ali-report`
- `ali-resume`
- `ali-revenue-operations`
- `ali-review`
- `ali-rfp-responder`
- `ali-risk-management-specialist`
- `ali-roadmap-communicator`
- `ali-run`
- `ali-runbook-generator`
- `ali-saas-metrics-coach`
- `ali-saas-scaffolder`
- `ali-sales-engineer`
- `ali-sample-skill`
- `ali-scenario-war-room`
- `ali-schema-markup`
- `ali-scrum-master`
- `ali-secrets-vault-manager`
- `ali-security-guidance`
- `ali-security-pen-testing`
- `ali-self-eval`
- `ali-self-improving-agent`
- `ali-seo-audit`
- `ali-setup`
- `ali-ship-gate`
- `ali-signup-flow-cro`
- `ali-site-architecture`
- `ali-skill-security-auditor`
- `ali-skill-tester`
- `ali-slo-architect`
- `ali-snowflake-development`
- `ali-soc2-audit-prep`
- `ali-soc2-compliance`
- `ali-spawn`
- `ali-spec-driven-workflow`
- `ali-spec-to-repo`
- `ali-sql-database-assistant`
- `ali-statistical-analyst`
- `ali-status`
- `ali-strategic-alignment`
- `ali-stress-test`
- `ali-stripe-integration-expert`
- `ali-syllabus`
- `ali-tc-tracker`
- `ali-tdd-guide`
- `ali-team-communications`
- `ali-tech-debt-tracker`
- `ali-tech-stack-evaluator`
- `ali-terraform-patterns`
- `ali-testrail`
- `ali-threat-detection`
- `ali-ui-design-system`
- `ali-universal-scraping-architect`
- `ali-ux-researcher-designer`
- `ali-vendor-management`
- `ali-video-content-strategist`
- `ali-vpe-advisor`
- `ali-vpe-review`
- `ali-webinar-marketing`
- `ali-workflow-builder`
- `ali-write-a-skill`
- `ali-x-twitter-growth`
- `ali-youtube-full`

---

## mutfak-dev

> Genel yazılım geliştirme (TDD, review, debug, mimari, deploy, dil) · **150 skill**

- `dev-agents-sdk`
- `dev-android-native-dev`
- `dev-api-and-interface-design`
- `dev-aspnet-core`
- `dev-brainstorming`
- `dev-browser-testing-with-devtools`
- `dev-buddy-sings`
- `dev-caveman`
- `dev-chatgpt-apps`
- `dev-ci-cd-and-automation`
- `dev-cli-creator`
- `dev-cloudflare`
- `dev-cloudflare-deploy`
- `dev-cloudflare-email-service`
- `dev-code-review-and-quality`
- `dev-code-simplification`
- `dev-color-font-skill`
- `dev-context-engineering`
- `dev-debugging-and-error-recovery`
- `dev-define-goal`
- `dev-deprecation-and-migration`
- `dev-design-an-interface`
- `dev-design-style-skill`
- `dev-development-lifecycle`
- `dev-diagnose`
- `dev-dispatching-parallel-agents`
- `dev-documentation-and-adrs`
- `dev-doubt-driven-development`
- `dev-durable-objects`
- `dev-edit-article`
- `dev-executing-plans`
- `dev-existing-project-onboarding`
- `dev-figma`
- `dev-figma-code-connect-components`
- `dev-figma-create-design-system-rules`
- `dev-figma-create-new-file`
- `dev-figma-generate-design`
- `dev-figma-generate-library`
- `dev-figma-implement-design`
- `dev-figma-use`
- `dev-finishing-a-development-branch`
- `dev-flutter-dev`
- `dev-frontend-dev`
- `dev-frontend-ui-engineering`
- `dev-fullstack-dev`
- `dev-gh-address-comments`
- `dev-gh-fix-ci`
- `dev-gif-sticker-maker`
- `dev-git-guardrails-claude-code`
- `dev-git-workflow-and-versioning`
- `dev-grill-me`
- `dev-grill-with-docs`
- `dev-handoff`
- `dev-hatch-pet`
- `dev-idea-refine`
- `dev-imagegen`
- `dev-improve-codebase-architecture`
- `dev-incremental-implementation`
- `dev-interview-me`
- `dev-ios-application-dev`
- `dev-jupyter-notebook`
- `dev-linear`
- `dev-low-level-design`
- `dev-mcp-builder`
- `dev-migrate-to-codex`
- `dev-migrate-to-shoehorn`
- `dev-minimax-docx`
- `dev-minimax-multimodal-toolkit`
- `dev-minimax-music-gen`
- `dev-minimax-music-playlist`
- `dev-minimax-pdf`
- `dev-minimax-xlsx`
- `dev-netlify-deploy`
- `dev-notion-knowledge-capture`
- `dev-notion-meeting-intelligence`
- `dev-notion-research-documentation`
- `dev-notion-spec-to-implementation`
- `dev-observability-and-instrumentation`
- `dev-obsidian-vault`
- `dev-openai-docs`
- `dev-openai-docs-openai`
- `dev-pdf`
- `dev-performance-optimization`
- `dev-planning-and-task-breakdown`
- `dev-playwright`
- `dev-playwright-interactive`
- `dev-plugin-creator`
- `dev-ppt-editing-skill`
- `dev-ppt-orchestra-skill`
- `dev-pptx-generator`
- `dev-pr-review`
- `dev-project-intake`
- `dev-prototype`
- `dev-qa`
- `dev-react-native-dev`
- `dev-receiving-code-review`
- `dev-render-deploy`
- `dev-request-refactor-plan`
- `dev-requesting-code-review`
- `dev-review`
- `dev-sandbox-sdk`
- `dev-scaffold-exercises`
- `dev-screenshot`
- `dev-security-and-hardening`
- `dev-security-best-practices`
- `dev-security-ownership-map`
- `dev-security-threat-model`
- `dev-sentry`
- `dev-setup-matt-pocock-skills`
- `dev-setup-pre-commit`
- `dev-shader-dev`
- `dev-shipping-and-launch`
- `dev-skill-creator`
- `dev-skill-creator-openai`
- `dev-skill-installer`
- `dev-slide-making-skill`
- `dev-source-driven-development`
- `dev-spec-driven-development`
- `dev-speech`
- `dev-subagent-driven-development`
- `dev-system-design-scaling`
- `dev-systematic-debugging`
- `dev-tdd`
- `dev-teach`
- `dev-test-driven-development`
- `dev-test-driven-development-obra`
- `dev-to-issues`
- `dev-to-prd`
- `dev-transcribe`
- `dev-triage`
- `dev-turnstile-spin`
- `dev-ubiquitous-language`
- `dev-using-agent-skills`
- `dev-using-git-worktrees`
- `dev-using-superpowers`
- `dev-vercel-deploy`
- `dev-verification-before-completion`
- `dev-vision-analysis`
- `dev-web-perf`
- `dev-winui-app`
- `dev-workers-best-practices`
- `dev-wrangler`
- `dev-write-a-skill`
- `dev-writing-beats`
- `dev-writing-fragments`
- `dev-writing-plans`
- `dev-writing-shape`
- `dev-writing-skills`
- `dev-yeet`
- `dev-zoom-out`

---

## mutfak-marketing

> Pazarlama, SEO, growth, içerik · **103 skill**

- `mkt-ab-test-setup`
- `mkt-ab-testing`
- `mkt-ad-creative`
- `mkt-ads`
- `mkt-aeo`
- `mkt-ai-seo`
- `mkt-analytics`
- `mkt-analytics-tracking`
- `mkt-app-store-optimization`
- `mkt-aso`
- `mkt-autoresearch`
- `mkt-brand-guidelines`
- `mkt-campaign-analytics`
- `mkt-churn-prevention`
- `mkt-clone-site`
- `mkt-closed-loop-analytics-upgrade`
- `mkt-co-marketing`
- `mkt-cold-email`
- `mkt-community-marketing`
- `mkt-competitor-alternatives`
- `mkt-competitor-profiling`
- `mkt-competitors`
- `mkt-content-creator`
- `mkt-content-eval`
- `mkt-content-humanizer`
- `mkt-content-ops`
- `mkt-content-production`
- `mkt-content-strategy`
- `mkt-conversion-ops`
- `mkt-copy-editing`
- `mkt-copywriting`
- `mkt-cro`
- `mkt-customer-research`
- `mkt-deck-generator`
- `mkt-directory-submissions`
- `mkt-email-sequence`
- `mkt-emails`
- `mkt-finance-ops`
- `mkt-form-cro`
- `mkt-free-tool-strategy`
- `mkt-free-tools`
- `mkt-growth-engine`
- `mkt-image`
- `mkt-launch`
- `mkt-launch-strategy`
- `mkt-lead-dossier`
- `mkt-lead-magnets`
- `mkt-marketing-context`
- `mkt-marketing-demand-acquisition`
- `mkt-marketing-ideas`
- `mkt-marketing-ops`
- `mkt-marketing-plan`
- `mkt-marketing-psychology`
- `mkt-marketing-skills`
- `mkt-marketing-strategy-pmm`
- `mkt-onboarding`
- `mkt-onboarding-cro`
- `mkt-outbound-engine`
- `mkt-page-cro`
- `mkt-paid-ads`
- `mkt-paywall-upgrade-cro`
- `mkt-paywalls`
- `mkt-podcast-ops`
- `mkt-popup-cro`
- `mkt-popups`
- `mkt-pricing`
- `mkt-pricing-strategy`
- `mkt-product-marketing`
- `mkt-programmatic-seo`
- `mkt-prompt-engineer-toolkit`
- `mkt-prospecting`
- `mkt-public-relations`
- `mkt-referral-program`
- `mkt-referrals`
- `mkt-revenue-intelligence`
- `mkt-revops`
- `mkt-sales-enablement`
- `mkt-sales-pipeline`
- `mkt-sales-playbook`
- `mkt-schema`
- `mkt-schema-markup`
- `mkt-seo-audit`
- `mkt-seo-ops`
- `mkt-short-form-pipeline`
- `mkt-signup`
- `mkt-signup-flow-cro`
- `mkt-site-architecture`
- `mkt-sms`
- `mkt-social`
- `mkt-social-content`
- `mkt-social-media-analyzer`
- `mkt-social-media-manager`
- `mkt-team-ops`
- `mkt-video`
- `mkt-video-caption-generator`
- `mkt-video-clip-pipeline`
- `mkt-video-content-strategist`
- `mkt-webinar-marketing`
- `mkt-x-longform-post`
- `mkt-x-twitter-growth`
- `mkt-youtube-full`
- `mkt-yt-competitive-analysis`
- `seo-technical`

---

## mutfak-pm

> Ürün yönetimi: strateji, PRD, metrik, keşif · **78 skill**

- `pm-ab-test-analysis`
- `pm-analyze-feature-requests`
- `pm-ansoff-matrix`
- `pm-beachhead-segment`
- `pm-brainstorm-experiments-existing`
- `pm-brainstorm-experiments-new`
- `pm-brainstorm-ideas-existing`
- `pm-brainstorm-ideas-new`
- `pm-brainstorm-okrs`
- `pm-business-model`
- `pm-cohort-analysis`
- `pm-company-values`
- `pm-competitive-battlecard`
- `pm-competitor-analysis`
- `pm-create-prd`
- `pm-customer-journey-map`
- `pm-draft-nda`
- `pm-dummy-dataset`
- `pm-find-community`
- `pm-first-customers`
- `pm-grammar-check`
- `pm-grow-sustainably`
- `pm-growth-loops`
- `pm-gtm-motions`
- `pm-gtm-strategy`
- `pm-ideal-customer-profile`
- `pm-identify-assumptions-existing`
- `pm-identify-assumptions-new`
- `pm-intended-vs-implemented`
- `pm-interview-script`
- `pm-job-stories`
- `pm-lean-canvas`
- `pm-market-segments`
- `pm-market-sizing`
- `pm-marketing-ideas`
- `pm-marketing-plan`
- `pm-metrics-dashboard`
- `pm-minimalist-review`
- `pm-monetization-strategy`
- `pm-mvp`
- `pm-north-star-metric`
- `pm-opportunity-solution-tree`
- `pm-outcome-roadmap`
- `pm-pestle-analysis`
- `pm-porters-five-forces`
- `pm-positioning-ideas`
- `pm-pre-mortem`
- `pm-pricing`
- `pm-pricing-strategy`
- `pm-prioritization-frameworks`
- `pm-prioritize-assumptions`
- `pm-prioritize-features`
- `pm-privacy-policy`
- `pm-processize`
- `pm-product-name`
- `pm-product-strategy`
- `pm-product-vision`
- `pm-release-notes`
- `pm-retro`
- `pm-review-resume`
- `pm-sentiment-analysis`
- `pm-shipping-artifacts`
- `pm-sprint-plan`
- `pm-sql-queries`
- `pm-stakeholder-map`
- `pm-startup-canvas`
- `pm-strategy-red-team`
- `pm-summarize-interview`
- `pm-summarize-meeting`
- `pm-swot-analysis`
- `pm-test-scenarios`
- `pm-user-personas`
- `pm-user-segmentation`
- `pm-user-stories`
- `pm-validate-idea`
- `pm-value-prop-statements`
- `pm-value-proposition`
- `pm-wwas`

---

## mutfak-design

> Vendor marka/ürün tasarım sistemleri · **99 skill**

- `design-ai-seo`
- `design-airbnb`
- `design-airtable`
- `design-amplitude`
- `design-anthropic`
- `design-apple`
- `design-arc`
- `design-ashby`
- `design-awwwards`
- `design-baseline-ui`
- `design-bear`
- `design-beehiiv`
- `design-brex`
- `design-cal`
- `design-clerk`
- `design-coda`
- `design-codeium`
- `design-convertkit`
- `design-convex`
- `design-copy-ai`
- `design-craft`
- `design-deel`
- `design-descript`
- `design-discord`
- `design-dribbble`
- `design-emil-design-eng`
- `design-figma`
- `design-fixing-accessibility`
- `design-fixing-metadata`
- `design-fixing-motion-performance`
- `design-framer`
- `design-front`
- `design-ghost`
- `design-github`
- `design-gumroad`
- `design-gusto`
- `design-hugging-face`
- `design-intercom`
- `design-interface-design`
- `design-jasper`
- `design-lattice`
- `design-launchdarkly`
- `design-lemonsqueezy`
- `design-linear`
- `design-liveblocks`
- `design-loom`
- `design-loops`
- `design-mailchimp`
- `design-matter`
- `design-mem`
- `design-mercury`
- `design-miro`
- `design-mixpanel`
- `design-mobbin`
- `design-monzo`
- `design-n26`
- `design-neon`
- `design-notion`
- `design-obsidian`
- `design-openai`
- `design-paddle`
- `design-pinterest`
- `design-pitch`
- `design-plaid`
- `design-planetscale`
- `design-posthog`
- `design-railway`
- `design-ramp`
- `design-raycast`
- `design-readwise`
- `design-replit`
- `design-resend`
- `design-retool`
- `design-reui-seo`
- `design-rippling`
- `design-robinhood`
- `design-segment`
- `design-seo-audit`
- `design-shadcn`
- `design-shopify`
- `design-shortcut`
- `design-slack`
- `design-spotify`
- `design-stripe`
- `design-substack`
- `design-supabase`
- `design-superhuman`
- `design-things`
- `design-ticktick`
- `design-tldraw`
- `design-todoist`
- `design-upstash`
- `design-vercel`
- `design-vercel-react-best-practices`
- `design-warp`
- `design-web-design-guidelines`
- `design-webflow`
- `design-weights-biases`
- `design-wise`

---

## mutfak-dotnet

> .NET test/diag/benchmark/migration skill'leri · **38 skill**

- `dev-dotnet-analyzing-dotnet-performance`
- `dev-dotnet-android-tombstone-symbolication`
- `dev-dotnet-apple-crash-symbolication`
- `dev-dotnet-architecture-selection`
- `dev-dotnet-assertion-quality`
- `dev-dotnet-clr-activation-debugging`
- `dev-dotnet-code-testing-agent`
- `dev-dotnet-code-testing-extensions`
- `dev-dotnet-coverage-analysis`
- `dev-dotnet-crap-score`
- `dev-dotnet-detect-static-dependencies`
- `dev-dotnet-dotnet-test-frameworks`
- `dev-dotnet-dotnet-trace-collect`
- `dev-dotnet-dump-collect`
- `dev-dotnet-filter-syntax`
- `dev-dotnet-generate-testability-wrappers`
- `dev-dotnet-grade-tests`
- `dev-dotnet-mcp-csharp-create`
- `dev-dotnet-mcp-csharp-debug`
- `dev-dotnet-mcp-csharp-publish`
- `dev-dotnet-mcp-csharp-test`
- `dev-dotnet-microbenchmarking`
- `dev-dotnet-migrate-mstest-v1v2-to-v3`
- `dev-dotnet-migrate-mstest-v3-to-v4`
- `dev-dotnet-migrate-static-to-wrapper`
- `dev-dotnet-migrate-vstest-to-mtp`
- `dev-dotnet-migrate-xunit-to-mstest`
- `dev-dotnet-migrate-xunit-to-xunit-v3`
- `dev-dotnet-mtp-hot-reload`
- `dev-dotnet-platform-detection`
- `dev-dotnet-run-tests`
- `dev-dotnet-technology-selection`
- `dev-dotnet-test-analysis-extensions`
- `dev-dotnet-test-anti-patterns`
- `dev-dotnet-test-gap-analysis`
- `dev-dotnet-test-smell-detection`
- `dev-dotnet-test-tagging`
- `dev-dotnet-writing-mstest-tests`

---

## mutfak-diagrams

> Markdown diyagram/görselleştirme · **15 skill**

- `md-archimate`
- `md-architecture`
- `md-bpmn`
- `md-canvas`
- `md-cloud`
- `md-data-analytics`
- `md-graphviz`
- `md-infocard`
- `md-infographic`
- `md-iot`
- `md-mindmap`
- `md-network`
- `md-security`
- `md-uml`
- `md-vega`

---

## mutfak-frontend

> Frontend/web premium UI, generative UI, test · **8 skill**

- `fe-generative-ui`
- `fe-premium-ui`
- `fe-uiux-audit`
- `theme-factory`
- `web-artifacts-builder`
- `web-design-engineer`
- `web-video-presentation`
- `webapp-testing`

---

## mutfak-research

> Akademik & derin araştırma · **4 skill**

- `research-academic-paper`
- `research-academic-paper-reviewer`
- `research-academic-pipeline`
- `research-deep-research`

---

## mutfak-core

> Çekirdek/genel (belge, görsel, artefakt, tekil) · **16 skill**

- `algorithmic-art`
- `beautiful-article`
- `brand-guidelines`
- `canvas-design`
- `claude-api`
- `doc-coauthoring`
- `docx`
- `frontend-design`
- `gpt-image-2`
- `internal-comms`
- `kb-retriever`
- `pdf`
- `pptx`
- `remotion`
- `slack-gif-creator`
- `xlsx`

---

